...
class OSRSBlast(OSRSBot):
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            ...
            # if not self.running:
            #     player_location = api.get_player_region_data()
            #     if player_location in tiles and player_location != self.last_location:
            #         self.last_location = player_location
            #         self.mouse.click()
            #         object_found = self.get_nearest_tag(tiles[player_location]['color'])
...
class Cerb:
    def __init__(self):
        self.base_endpoint = "http://localhost:8085/"
        self.cerb_endpoint = "zammy"
        self.timeout = 1
