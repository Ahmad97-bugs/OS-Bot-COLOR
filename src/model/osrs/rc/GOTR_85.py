...
from src.model.osrs.rc.Location import get_location, Location
...
class OSRSGOTR(OSRSBot):
    ...
    def click_altar(self, api_m: MorgHTTPSocket):
        ...
        while True:
            self.empty_pouches(api_m)
            self.select_color(clr.GREEN, 'Craft', api_m)
            api_m.wait_til_gained_xp("Runecraft", 3)
            if not self.colossal_pouch:
                break
            self.log_msg("Crafting another set of essence..")
    ...
    def blue_portal_sequence(self, api_m: MorgHTTPSocket, wait):
        ...
        if api_m.get_if_item_in_inv(ids.VARROCK_ARMOUR_4):
            self.mouse.move_to(self.win.inventory_slots[1].random_point(), mouseSpeed='fastest')
            self.mouse.click()
        while True:
            if not (remains := self.get_nearest_tag(clr.RED)):
                time.sleep(3)
            self.log_msg(f'Colossal pouch: {self.colossal_pouch}')
    ...
    def choose_altar(self):
        pillar_images = ['blood', 'fire', 'death', 'earth', 'law', 'nature', 'cosmic', 'chaos', 'water', 'air', 'body', 'mind']
    ...
    def fill_pouches(self, api_m: MorgHTTPSocket):
        self.update_pouch(len(api_m.get_inv_item_indices(ids.GUARDIAN_ESSENCE)))
        self.mouse.move_to(self.win.inventory_slots[0].random_point(), mouseSpeed='fastest')
        self.mouse.click()

    def empty_pouches(self, api_m):
        pag.keyDown('shift')
        self.mouse.move_to(self.win.inventory_slots[0].random_point(), mouseSpeed='fastest')
        self.mouse.click()
        pag.keyUp('shift')
        self.update_pouch(0 - len(api_m.get_inv_item_indices(-1)))
