...
class OSRSB(OSRSBot):
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            self.open_bank()
            self.refresh_essence(api_m)
            self.withdraw_from_bank(["Pure_essence_bank"], False, False)
            self.use_pouch()

            self.withdraw_from_bank(["Pure_essence_bank"], False, False)
            self.use_pouch()

            self.withdraw_from_bank(["Pure_essence_bank"], False, True)
            if api_m.get_if_item_in_inv([ids.BLOOD_ESSENCE]):
                self.mouse.move_to(self.win.inventory_slots[1].random_point(), mouseSpeed='fastest')
                self.mouse.click()
            self.repair_pouches(api_m)

            self.mouse.move_to(self.win.inventory_slots[5].random_point(), mouseSpeed='fastest')
            self.mouse.click()
            self.sleep(2700, 3000)

            self.refresh_energy(api_m)

            self.select_color(clr.RED, ['Last', '-', 'destination', '(DLS)'], api_m)
            wait(api_m, (3447, 9824, 0))
            self.sleep(1200, 1500)
            self.select_color(clr.GREEN, ['Enter'], api_m)
            self.sleep(2500, 3000)
            wait(api_m, (3460, 9813, 0))
            self.select_color(clr.YELLOW, ['Enter'], api_m)

            self.sleep(2000, 2500)
            self.map_search('blood_altar_step_1')
            self.map_search('blood_altar_step_2')
            while api_m.get_animation_id() not in [808, 813]:
                continue
            self.select_color(clr.GREEN, ['Enter'], api_m)
            wait(api_m, (3536, 9768, 0))
            self.select_color(clr.YELLOW, ['Enter'], api_m)
            wait(api_m, (3543, 9772, 0))
            self.select_color(clr.RED, ['Enter'], api_m)
