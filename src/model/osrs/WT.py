...
class OSRSwintertodt(OSRSBot):
    ...
    def main_loop(self):
        ...
        # self.select_color(clr.RED, 'Chop', api_m)
        object_found = self.get_nearest_tag(clr.GREEN)
        self.mouse.move_to(object_found.random_point())
        self.mouse.click()
        while api_m.get_player_position() != (2946, 3370, 0):
            time.sleep(0.1)
        true_value = time.time()
        self.log_msg(f'landed at {true_value}')
        while not api_m.get_is_player_idle(0.1):
            time.sleep(0.1)
        idle_value = time.time()
        self.log_msg(f'became idle at {idle_value}')

        self.log_msg(idle_value - true_value)
    ...
    def bank_items(self):
        supply_crate_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "supply_crate.png")
        supply_crate = imsearch.search_img_in_rect(supply_crate_img, self.win.control_panel)
        shark_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "Jug_of_wine.png")
        shark_bank_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "Jug_of_wine_bank.png")
    ...
    def cut_logs(self):
        idle_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "IDLE.png")
        idle = imsearch.search_img_in_rect(idle_img, self.win.game_view)
        woodcutting_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "woodcutting.png")
    ...
    def fletch_logs(self):
        knife_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "knife.png")
        knife = imsearch.search_img_in_rect(knife_img, self.win.control_panel)
        bruma_root_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "bruma_root.png")
    ...
    def firemake(self):
        bruma_kindling_img = imsearch.BOT_IMAGES.joinpath("winter_todt", "bruma_kindling.png")
    ...
    def search_slot_28(self):
        ...
        item_path = imsearch.BOT_IMAGES.joinpath("winter_todt", "emptyslot.PNG")
