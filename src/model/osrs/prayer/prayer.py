...
class OSRSB(OSRSBot):
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            ...
            self.open_bank()
            self.withdraw_from_bank(["Marrentill_bank"], True, False)
            self.withdraw_from_bank(["Marrentill_bank"], True, False)

            self.withdraw_from_bank(["Wyrm_bones_bank"], False)
            self.mouse.move_to(self.win.inventory_slots[1].random_point(), mouseSpeed='fastest')
            self.mouse.click()
            self.sleep(2700, 3000)

            self.select_color(clr.BLUE, 'Light', api_m)
            while self.get_nearest_tag(clr.BLUE):
                time.sleep(2)
            self.select_color(clr.GREEN, 'Light', api_m)
            time.sleep(5)

            if api_m.get_if_item_in_inv(ids.MARRENTILL):
                self.select_color(clr.GREEN, 'Light', api_m)
                time.sleep(2)

            self.mouse.move_to(self.win.inventory_slots[5].random_point(), mouseSpeed='fastest')
            self.mouse.click()
            while not (object_found := self.get_nearest_tag(clr.PINK)):
                time.sleep(0.1)
            self.mouse.move_to(object_found.random_point(), mouseSpeed='fastest')
            self.sleep(700, 915)
            self.mouse.click()

            while api_m.wait_til_gained_xp('Prayer', 5) == -1:
                self.mouse.move_to(self.win.inventory_slots[5].random_point(), mouseSpeed='fastest')
                self.mouse.click()
                while not (object_found := self.get_nearest_tag(clr.PINK)):
                    time.sleep(0.1)
            ...
            while api_m.get_if_item_in_inv([ids.WYRM_BONES]):
                time.sleep(2)
